...
# AJUSTADO (mesmo fix do Tokyo/Starcheck):
# - Índice NÃO é mais silencioso: mostra spinner e ERRO REAL
# - Aceita colunas URL/LINK/PLANILHA/ARQUIVO e ATIVO/ATIVA/STATUS
# - Cache (índice + leitura das planilhas) para não reler tudo a cada interação
# - Se algumas planilhas falharem, lista quais falharam (sem travar se houver dados suficientes)
# ------------------------------------------------------------
...
st.title("Produção por Vistoriador - LOG VISTORIAS")
...
# =========================
# Cache: leitura da planilha (por sheet_id)
# =========================
...
# =========================
# Índice: leitura robusta (com erro real)
# =========================
def _yes(v) -> bool:
...
def load_ids_from_index_cached(index_sheet_id: str, tab_name: str) -> List[str]:
    ...
    # Aba: tenta exata, depois match por upper
    try:
...
if not sheet_ids:
    st.error("Nenhum arquivo ATIVO com URL/ID válido na aba ARQUIVOS do índice.")
    st.stop()
...
# metas: usa o mês selecionado
ref_ym = f"{ref_year}-{ref_month:02d}"
metas_ref = df_metas_all[df_metas_all["__YM__"] == ref_ym].copy() if (not df_metas_all.empty and "__YM__" in df_metas_all.columns) else pd.DataFrame()
...
fmt["NECESSIDADE_DIA"] = grp_tbl["NECESSIDADE_DIA"].map(lambda x: "0" if x <= 0 else f"{int(round(x))}")
fmt["TENDÊNCIA"]       = grp_tbl["TENDENCIA_%"].map(lambda p: "—" if pd.isna(p) else f"{float(p):.0f}%")
...
if fmt.empty or not cols_show_avail:
    ...
    st.download_button("Baixar resumo (CSV)", data=csv, file_name="resumo_vistoriador_log.csv", mime="text/csv")
...
if len(datas_ok) == 0:
    ...
    cards_mes = [
        ...
        ("% Ating. (geral)", "—" if pd.isna(ating_g) else f"{float(ating_g):.0f}%"),
    ]
    st.markdown(
        '<div class="card-container">' +
    ...
    def render_ranking(df_sub, titulo):
        ...
        with c1:
            ...
            st.dataframe(top[["VISTORIADOR","META_MENSAL","VISTORIAS","REVISTORIAS","LIQUIDO","ATING_%"]], use_container_width=True, hide_index=True)
        with c2:
            st.markdown(f"**BOTTOM BOX — {mes_label}**")
            st.dataframe(bot[["VISTORIADOR","META_MENSAL","VISTORIAS","REVISTORIAS","LIQUIDO","ATING_%"]], use_container_width=True, hide_index=True)
...
if not dates_avail:
    ...
    prod_dia = (view_dia.groupby("VISTORIADOR", dropna=False)
                .agg(VISTORIAS_DIA=("IS_REV","size"), REVISTORIAS_DIA=("IS_REV","sum")).reset_index())
    ...
    def render_ranking_dia(df_sub, titulo):
        ...
        with c1:
            st.markdown("**TOP BOX**")
            st.dataframe(top[["VISTORIADOR","META_DIA","VISTORIAS_DIA","REVISTORIAS_DIA","LIQUIDO_DIA","ATING_DIA_%"]],
                         use_container_width=True, hide_index=True)
        with c2:
            st.markdown("**BOTTOM BOX**")
            st.dataframe(bot[["VISTORIADOR","META_DIA","VISTORIAS_DIA","REVISTORIAS_DIA","LIQUIDO_DIA","ATING_DIA_%"]],
                         use_container_width=True, hide_index=True)
